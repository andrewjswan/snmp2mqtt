...
export const createClient = async (
    ...
    return {
        ...
        sensorStatusTopic: (sensor: SensorConfig, target: TargetConfig) =>
            `${config.base_topic}/${config.host_name_as_target && target.name ? slugify(target.name) : target.host}/${slugify(sensor.name)}/status`,
        sensorValueTopic: (sensor: SensorConfig, target: TargetConfig) =>
            `${config.base_topic}/${config.host_name_as_target && target.name ? slugify(target.name) : target.host}/${slugify(sensor.name)}/value`,
